...
export function BranchView({ branches }: BranchViewProps) {
  ...
  // Auto-reset add form on success
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Manage physical clinic locations, contact details, operating hours, and active status.
          </p>
        </div>
        <button
          type="button"
          aria-expanded={isAdding}
          aria-controls="add-branch-accordion"
          onClick={() => {
        ...
        >
          {isAdding ? "Cancel" : "+ Add New Branch"}
        </button>
      </div>

      {/* Add Branch Accordion Form */}
      {isAdding && (
        <div
          id="add-branch-accordion"
          className="bg-white p-6 rounded-2xl border border-orange-200/80 shadow-md space-y-4"
        >
          <h2 className="text-base font-bold text-navy-900">Add New Clinic Branch</h2>
          ...
          <form ref={addFormRef} action={createAction} className="space-y-4 max-w-3xl">
            ...
            <div className="pt-2">
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-navy-900 hover:bg-navy-800 text-white font-bold text-xs shadow-xs transition-colors cursor-pointer disabled:opacity-50"
              >
      ...
      {editingBranch && (
        <div
          ...
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/40 backdrop-blur-xs"
        >
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-2xl w-full max-w-xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <button
                ...
                className="text-slate-400 hover:text-navy-900 p-1.5 rounded-lg text-sm"
